...
:error_location
echo Doesn't seem like this script is in a GoAnimate Offline folder.
pause && exit
:noerror_location
...
if !FLASH_DETECTED!==n (
	...
	echo GoAnimate Offline will not continue installation until you press a key.
	echo:
	pause
	echo:
...
if !HTTPSCERT_DETECTED!==n (
	...
	if /i not "!SAFE_MODE!"=="y" (
		...
		if /i not !ERRORLEVEL!==0 (
			...
			echo ^(do this by right-clicking start_goanimateoffline.bat and click "Run as Administrator"^)
			echo:
...
if !ADMINREQUIRED!==y (
	...
	echo:
	echo GoAnimate Offline no longer needs Admin rights,
...
color 0f
echo All dependencies now installed^^! Continuing with GoAnimate Offline boot.
echo:
