...
:error_location
echo Doesn't seem like this script is in a GoAniFire: Off Duty folder.
pause && exit
:noerror_location
...
if !FLASH_DETECTED!==n (
	...
	if !BROWSER_TYPE!==n (
		...
		echo GoAniFire: Off Duty will not be able to install Flash.
	...
	if !NODEJS_DETECTED!==y (
		...
		if !errorlevel! == 0 (
			...
			if not exist "utilities\installers\http-server-master" (
				...
				echo A normal copy of GoAniFire: Off Duty should come with them.
				echo You should be able to find a copy on this website:
...
if !ADMINREQUIRED!==y (
	...
	echo:
	echo GoAniFire: Off Duty no longer needs Admin rights,
	echo please restart normally by double-clicking.
	echo:
	echo If you saw this from running normally,
	echo GoAniFire: Off Duty should continue normally after a restart.
	echo:
	if !DRYRUN!==y (
...
color 0f
echo All dependencies now installed^^! Continuing with GoAniFire: Off Duty boot.
echo:
...
echo Enter 0 to close GoAniFire: Off Duty
...
title GoAniFire: Off Duty v!WRAPPER_VER! [Shutting down...]
